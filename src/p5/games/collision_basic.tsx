...
function collisionB(p5: P5CanvasInstance) {
  let canvas;
  let bg = 150;
  let boardColor = 40;
  ...
  class World {
    ...
    checker(ball: any, ball2: any){
      console.log("Here");
                let x1 = ball.pos.x;
                let y1 = ball.pos.y;
                let x2 = ball2.pos.x;
                let y2 = ball2.pos.y;
                let dist = p5.dist(x1, x2, y1, y2);
                let minDist = ball.r + ball2.r;
    ...
    update() {
      ...
      p5.keyIsDown(65)
        ? !this.addState
          ? (this.checks.push(
              new Ball(p5.random(p5.width / 3, 2*p5.width/3), p5.height / 3, p5.random(20, 40))
            ),
            (this.addState = true))
          : null
        : (this.addState = false);
...
export default function CollisionB() {
  return <ReactP5Wrapper sketch={collisionB} />;
}
